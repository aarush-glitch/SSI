...
import { submitAntibioticForm } from './api/nurseFormApi';
...
const AntibioticForm = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await submitAntibioticForm(antibiotic_form);
      alert('Antibiotic data submitted successfully!');
    } catch (error) {
  ...
  const handleDownloadPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(14);
    doc.text('Surgical Timing Monitoring Form', 10, 10);
  ...
  return (
    <form onSubmit={handleSubmit} style={styles.antibioticForm}>
      <h2>Surgical Timing Monitoring Form</h2>
